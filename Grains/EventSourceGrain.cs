...
namespace Grains
{
    public abstract  class EventSourceGrain<TGrainState,TEventBase>:JournaledGrain<TGrainState,TEventBase>,
        ICustomStorageInterface<TGrainState,TEventBase> where TGrainState:class,new() where TEventBase:class
    {
        private readonly IEventStoreConnection _eventStoreConnection;
        private readonly JsonSerializerSettings _jsonSerializerSettings;
        protected abstract string GetGrainKey();

        protected EventSourceGrain(IEventStoreConnection eventStoreConnection,JsonSerializerSettings jsonSerializerSettings)
        {
            _eventStoreConnection = eventStoreConnection;
            _jsonSerializerSettings = jsonSerializerSettings;
        }

        public new async Task<IEnumerable<TEventBase>> RetrieveConfirmedEvents(int fromVersion, int toVersion)
        {
            var resolvedEvents=new List<TEventBase>();
            var allEventSlice =
                await _eventStoreConnection.ReadStreamEventsForwardAsync(GetStreamName(), fromVersion, toVersion,
                    false);
            foreach (var resolvedEvent in allEventSlice.Events)
            {
        ...
        public async Task<KeyValuePair<int, TGrainState>> ReadStateFromStorage()
        {
            ...
            {
                var allEventSlice =
                    await _eventStoreConnection.ReadStreamEventsForwardAsync(GetStreamName(), 0L, 100, false);
                foreach (var resolvedEvent in allEventSlice.Events)
                {
        ...
        public virtual async Task<bool> ApplyUpdatesToStorage(IReadOnlyList<TEventBase> updates, int expectedversion)
        {
            ...
            {
                throw new ArgumentException(GetGrainKey());
            }
            foreach (var update in updates)
            {
                try
                {
                    ...
                    await _eventStoreConnection.AppendToStreamAsync(GetStreamName(), ExpectedVersion.Any, eventData);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
        ...
        private string GetStreamName()
        {
            ...
            var stream = "OurGreetingsStream:{0}:{1}".Replace("{0}", GetType().Name).Replace("{1}", grainKey);
            return stream;
        }
